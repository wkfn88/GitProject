...
import Seller.SellerMain;
import customer.Main0;
...
public class Login {
	...
	private void initialize() {
		...
		bt_login.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				if( rb_consumer.isSelected() ) {
					LoginDAO dao = new LoginDAO();
					ArrayList<MemberVo> list = dao.cusList();
					
					if(list.size() == 0 ) {
						JOptionPane.showMessageDialog(frame, "아이디가 존재하지 않습니다.", "오류", JOptionPane.ERROR_MESSAGE);
					}else {
						for(int i = 0; i < list.size(); i++ ) {
							String id = list.get(i).getId();
							String pwd = list.get(i).getPassword();
							if( tf_id.getText().equals(id) && tf_password.getText().equals(pwd)) {	
								//사용자 ui로 넘어가는 생성자
								new Main0();
								frame.dispose();
								break;
